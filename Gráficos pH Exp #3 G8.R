...
df_pler2 <- read_excel("Gráfico pH G8.xlsx", sheet = "PLER 2026-2")
...
ggplot(df_pler2_bar, aes(x = `Time (days)`, y = pH)) +
...
df_pler2lm <- read_excel("Gráfico pH G8.xlsx", sheet = "PLER 2026-2 LM")
...
df_pysa <- read_excel("Gráfico pH G8.xlsx", sheet = "PYSA")
...
ggplot(df_pysa_bar, aes(x = `Time (days)`, y = pH)) +
  geom_line(aes(group = Treatment, color = Treatment)) +
  geom_point(aes(shape = Treatment, fill = Treatment),
             color = "black",
             size = 4,
...
## Cargamos un nuevo set de datos y transformamos las columnas para un LM
df_pysalm <- read_excel("Gráfico pH G8.xlsx", sheet = "PYSA LM")
