...
df_ph_unificado <- read_excel("Gráfico pH G8.xlsx", sheet = "UNIFICADOS")

## Transformamos las columnas categóricas a factores, y el pH a numérica
df_ph_unificado$Species <- factor(df_ph_unificado$Species)
df_ph_unificado$Treatment <- factor(df_ph_unificado$Treatment)
df_ph_unificado$`Time (days)` <- factor(df_ph_unificado$`Time (days)`)
df_ph_unificado$Replicate <- factor(df_ph_unificado$Replicate)

## Creamos un dataframe con errores estándares
df_ph_unificado_bar <- df_ph_unificado %>%
...
ggplot(df_ph_unificado_bar, aes(x = `Time (days)`, y = pH)) +
  geom_line(aes(group = Species, color = Species)) +
  geom_point(aes(shape = Species, fill = Species),
             color = "black",
             size = 4,
             stroke = 0.8) +
  geom_errorbar(aes(ymin = pH - se, ymax = pH + se, color = "black"), 
                width = 0.8,
                linewidth = 0.6) +
